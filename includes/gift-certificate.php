...
function bookitfast_render_gift_certificate_block( $attributes ) {
    ...
    if ( ! isset( $settings['enable_gift_certificates'] ) || ! $settings['enable_gift_certificates'] ) {
        return '<div class="alert alert-danger">Sorry, but Gift Certificates Are Not Enabled</div>';
    }
    ...
    ob_start();
    ?>
    <div class="bookitfast-certificate-form"
        ...
        <div id="gc-error-container" class="alert alert-danger" style="display: none;"></div>
        <form id="gift-certificate-purchase-form" method="post" action="<?php echo esc_url( admin_url('admin-post.php') ); ?>">
            <!-- Gift Certificate Details -->
            <h4>Gift Certificate Details</h4>
            <div class="form-group form-row">
                <label for="gc_amount">Amount:</label>
                <?php if ( $allowCustom ) : ?>
                    <input type="number" id="gc_amount" name="gc_amount" step="0.01" class="form-control" required
                           placeholder="Between <?php echo esc_attr( $min ); ?> and <?php echo esc_attr( $max ); ?>">
                <?php else : ?>
                    <select id="gc_amount" name="gc_amount" class="form-control" required>
            ...
            <div class="form-group form-row">
                ...
                <input type="text" id="gc_to" name="gc_to" class="form-control" required>
            </div>
            <div class="form-group form-row">
                <label for="gc_from">From (Displayed on Certificate):</label>
                <input type="text" id="gc_from" name="gc_from" class="form-control" required>
            </div>
            <div class="form-group form-row">
                <label for="gc_message">Message:</label>
                <input type="text" id="gc_message" name="gc_message" class="form-control" required>
            </div>
            ...
            <div class="form-group form-row">
                ...
                <input type="text" id="recipient_first_name" name="recipient_first_name" class="form-control" required>
            </div>
            <div class="form-group form-row">
                <label for="recipient_last_name">Recipient Last Name:</label>
                <input type="text" id="recipient_last_name" name="recipient_last_name" class="form-control" required>
            </div>
            ...
            <div class="user-details-form">
                <div class="form-group form-row">
                    ...
                    <input type="text" id="customer_first_name" name="customer_first_name" class="form-control" required>
                </div>
                <div class="form-group form-row">
            ...
            <div id="stripe-gc-payment">
                <!-- Stripe Elements container will be injected here if a payment is required -->
            </div>

            <button type="button" id="gc-proceed-button" class="btn btn-rounded" style="background-color: <?php echo esc_attr( $buttonColor ); ?>;">
                Proceed to Payment
            </button>
        </form>
    </div>
